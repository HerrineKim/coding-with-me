...
export const studentVideoData = defineStore("studentVideoData", () => {
  ...
  const getStudentList = async () => {
    let result = await api.get(
      `/student/${piniaCommonVideoData.userInfo.userKey}/classes`
    );

    piniaCommonVideoData.displayInfo.studentList.splice(
      0,
      piniaCommonVideoData.displayInfo.studentList.length
    );

    for (let item of result.data.students) {
      piniaCommonVideoData.displayInfo.studentList.push(item.name);
    }
    piniaCommonVideoData.displayInfo.studentListIsActive = Array.apply(
      null,
      Array(piniaCommonVideoData.displayInfo.studentList.length)
    ).map(() => false);

    let myIdx = piniaCommonVideoData.displayInfo.studentList.indexOf(
      piniaCommonVideoData.userInfo.userName
    );
    if (myIdx !== -1) {
